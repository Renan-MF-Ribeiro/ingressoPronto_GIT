...
export class Auth {
    ...
    public registerLogin(tokem: string) {
        sessionStorage.setItem(btoa('token'), tokem)
    }
    public registerLevel(level: string) {
        ...
        firebase.auth().createUserWithEmailAndPassword(user.email, user.password)
            .then((resposta: any) => {
                ...
                firebase.database().ref(`usuario_detalhe/${btoa(user.email)}`)
                    .set(user).then((sucess:any)=>{
                        this.cadastroSucess = 'true'
                    })
            ...
            .then((resposta: any) => {
                ...
                firebase.auth().currentUser?.getIdToken()
                    .then((idToken: string) => {
                        ...
                        sessionStorage.setItem(btoa('token'), idToken)
...
VerifyErroCode(errorCode: string): string {
            ...
            default:
                return 'Error não indentificado';
        }
    }
}